...
export class ResponsiveTester {
  ...
  /**
   * Compare with reference site using MCP visual diff
   */
  async compareWithReference() {
    ...
    this.logger.info(`Comparing with reference: ${this.config.referenceUrl}`);
    ...
    try {
      ...
      // Compare each viewport screenshot with reference
      for (const viewportResult of this.results.viewports) {
        if (!viewportResult.screenshot) continue;

        // Capture reference at same viewport
        const page = await browser.newPage();
        await page.setViewport({ width: viewportResult.viewport.width, height: viewportResult.viewport.height });
        ...
        const refScreenshot = path.join(
          this.config.screenshotsDir,
          `reference-${viewportResult.viewport.name.replace(/\s+/g, '-').toLowerCase()}-${viewportResult.viewport.width}x${viewportResult.viewport.height}.png`
        );
        await page.screenshot({ path: refScreenshot, fullPage: true });
        await page.close();

        // Run visual diff
        const diff = await this.mcp.visualDiff(refScreenshot, viewportResult.screenshot, {
          prompt: `Compare responsive layouts at ${viewportResult.viewport.width}x${viewportResult.viewport.height}. Report specific CSS differences for mobile/tablet/desktop.`
        });

        comparisons.push({
          viewport: viewportResult.viewport.name,
          width: viewportResult.viewport.width,
          height: viewportResult.viewport.height,
          matchAchieved: diff.matchAchieved || false,
          matchPercentage: diff.matchPercentage || 0,
          issues: diff.differences || [],
          fixes: diff.fixes || []
        });
      }
      ...
      return {
        ...
        summary: {
          ...
          matched: comparisons.filter(c => c.matchAchieved).length,
          avgMatchPercentage: Math.round(comparisons.reduce((sum, c) => sum + c.matchPercentage, 0) / comparisons.length)
        }
      };

    } catch (error) {
      this.logger.error(`Reference comparison failed: ${error.message}`);
      return {
        url: this.config.referenceUrl,
        error: error.message,
        comparisons: []
