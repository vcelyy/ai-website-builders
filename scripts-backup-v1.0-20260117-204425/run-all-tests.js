...
export class TestSuite {
  ...
  async runPhase3_SlowSequential() {
    ...
    if (this.config.tests.responsive.enabled) {
      ...
      const responsiveStatus = result.summary.failed === 0 ? 'PASSED' : `FAILED (${result.summary.failed} viewport(s) failed)`;
      logger.info(`  Responsive test: ${responsiveStatus}`);
    }
    ...
    if (this.config.tests.crossBrowser.enabled) {
      ...
      const browserStatus = result.summary.failed === 0 ? 'PASSED' : `FAILED (${result.summary.failed} browser(s) failed)`;
      logger.info(`  Cross-browser test: ${browserStatus}`);
    }
  }
