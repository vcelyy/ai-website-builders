...
export class ImageValidator {
  ...
  async validateImage(image) {
    ...
    try {
      ...
      // Test 6: Accessibility analysis via MCP vision tool
      try {
        const a11yResult = await this.analyzeImageAccessibility(image.src, image.alt);
        result.metadata.a11yTest = a11yResult;

        if (!a11yResult.passed) {
          result.issues.push({
            type: 'accessibility_issue',
            severity: a11yResult.severity || 'warning',
            message: a11yResult.message
          });
        }
      } catch (a11yError) {
        // MCP analysis failed, but don't fail the entire test
        this.logger.debug(`  Accessibility analysis unavailable: ${a11yError.message}`);
      }
  ...
  /**
   * Analyze image for accessibility issues using MCP vision tool
   */
  async analyzeImageAccessibility(imageSrc, currentAlt) {
    try {
      const analysis = await this.mcp.analyzeImage(
        imageSrc,
        `Analyze this image for accessibility issues:
        1. Is alt text needed? (decorative images don't need alt)
        2. Is the current alt text adequate? Current: "${currentAlt || '(none)'}"
        3. Any contrast or visibility issues?
        4. Is this a meaningful image or decorative?

        Report specific issues and severity levels.`
      );

      // Parse MCP response for accessibility issues
      ...
      // If MCP indicates alt text is needed and missing
      if (analysis && analysis.needsAlt && !currentAlt) {
        return {
          passed: false,
          severity: 'critical',
          message: 'Alt text required for this meaningful image',
          suggestedAlt: analysis.suggestedAltText || ''
        };
      }

      return {
        passed: true,
        message: 'Image accessibility acceptable'
      };

    } catch (error) {
      // If MCP fails, return neutral result (don't fail the test)
      return {
        passed: true,
        message: 'Accessibility analysis skipped (MCP unavailable)'
      };
    }
  }
